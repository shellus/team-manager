...
export class PublicSeatService {
  ...
  async swap(seatKey: string, rawEmail: string): Promise<PublicSeatSlotView> {
    ...
    if (row.normalized_current_email === email && ['member', 'invited'].includes(row.status)) return seatView(row);
    ...
    const operation = await this.db.insertInto('seat_slot_swap_operations').values({
      ...
      requested_email: email, error_message: null
    ...
    try {
      ...
      await this.db.transaction().execute(async (trx) => {
        ...
        await trx.updateTable('seat_slot_swap_operations').set({ status: 'succeeded' }).where('id', '=', operation.id).execute();
      });
