...
export class TeamOrderService {
  ...
  private async config(row:any){const specific=await this.db.selectFrom('team_order_configurations').selectAll().where('workspace_id','=',row.workspace_id).executeTakeFirst(); const global=await this.db.selectFrom('team_order_configurations').selectAll().where('workspace_id','is',null).executeTakeFirst(); const config={promoCode:row.promo_code??specific?.promo_code??global?.promo_code??'',country:(row.country??specific?.country??global?.country??'').toUpperCase(),currency:(row.currency??specific?.currency??global?.currency??'').toUpperCase()}; if(!/^[A-Z]{2}$/.test(config.country)||!/^[A-Z]{3}$/.test(config.currency)) throw new ServiceError(400,'订单国家或货币配置无效'); return config;}
