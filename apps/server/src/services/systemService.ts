...
export class SystemService {
  ...
  async overviewSeats(): Promise<SeatOperationalOverviewView[]> {
    ...
    const byWorkspace = new Map(workspaces.map((row) => [row.id, row])); const result: SeatOperationalOverviewView[] = [];
    for (const row of memberships) { const workspace=byWorkspace.get(row.workspace_id);if(!workspace)continue;result.push(seatView(workspace,'membership',row.id,row.seat_type,row.status,{email:row.email??undefined,displayName:row.display_name??undefined,role:row.normalized_role})); }
    for (const row of invitations) { const workspace=byWorkspace.get(row.workspace_id);if(!workspace)continue;result.push(seatView(workspace,'invitation',row.id,row.seat_type,row.status,{email:row.email,role:row.normalized_role})); }
    for (const row of slots) { const workspace=byWorkspace.get(row.workspace_id);if(!workspace)continue;result.push(seatView(workspace,'seat_slot',row.id,row.seat_type,row.status,{email:row.current_email??undefined,contact:row.contact??undefined,remark:row.remark??undefined,expiresOn:row.expires_on??undefined,price:row.price??undefined})); }
    ...
    return result.sort((a,b)=>riskRank(a.riskLevel)-riskRank(b.riskLevel)||(a.expiresOn??'9999').localeCompare(b.expiresOn??'9999'));
  }
}
...
function seatView(workspace:any,source:SeatOperationalOverviewView['source'],id:string,seatType:unknown,status:string,extra:any):SeatOperationalOverviewView{const risks:string[]=[];if(source==='fixed_vacancy')risks.push('固定 ChatGPT 空位');if(extra.expiresOn&&extra.expiresOn<new Date().toISOString().slice(0,10))risks.push('客户席位已到期');return{id,workspaceId:workspace.id,...(workspace.name?{workspaceName:workspace.name}:{}),workspaceExternalId:workspace.external_id,source,...extra,seatType:seatType==='usage_based'?'usage_based':'default',status,riskLevel:risks.some(item=>item.includes('到期'))?'critical':risks.length?'warning':'normal',risks};}
