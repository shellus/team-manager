...
export function seatRisks(expiresOn:string|undefined,workspaceStatus:string,lacksManager:boolean,now=new Date(),trackCustomerExpiry=true){const risks:string[]=[];const today=seatExpirationBusinessDate(now);const soon=addCalendarDays(today,3);if(trackCustomerExpiry){if(!expiresOn)risks.push('未设置到期日');else if(expiresOn<today)risks.push('客户席位已到期');else if(expiresOn<=soon)risks.push('三天内到期');}if(workspaceStatus!=='active')risks.push('Workspace 非活动');if(lacksManager)risks.push('缺少可管理账号');return risks;}
