...
export class CredentialService {
  ...
  async replace(id: string, content: unknown) {
    ...
    await this.db.updateTable('workspace_credentials').set({ status: 'disabled', disabled_at: new Date() }).where('id', '=', id).execute();
    await this.#activity.log({accountId:row.account_id,workspaceId:row.workspace_id,kind:'credential_replaced',payload:{previousCredentialId:id,credentialId:saved.id}});return saved;
  }
  async setStatus(id: string, status: 'active' | 'disabled' | 'revoked') { const row = await this.db.updateTable('workspace_credentials').set({ status, disabled_at: status === 'active' ? null : new Date() }).where('id', '=', id).returningAll().executeTakeFirst(); if (!row) throw new ServiceError(404, '凭证不存在'); await this.#activity.log({accountId:row.account_id,workspaceId:row.workspace_id,kind:'credential_status_changed',payload:{credentialId:id,status}});return row; }
  async remove(id: string) { const row = await this.db.selectFrom('workspace_credentials').selectAll().where('id', '=', id).executeTakeFirst(); if (!row) throw new ServiceError(404, '凭证不存在'); if (row.status === 'active') throw new ServiceError(409, '活动凭证必须先停用'); await this.db.deleteFrom('workspace_credentials').where('id', '=', id).execute();await this.#activity.log({accountId:row.account_id,workspaceId:row.workspace_id,kind:'credential_removed',payload:{credentialId:id,credentialKind:row.kind}}); return true; }
  ...
  async completeOauth(sessionId: string, callbackUrl: string, poolGroupId?: string) {
    ...
    if (poolGroupId && !await this.db.selectFrom('credential_pool_groups').select('id').where('id', '=', poolGroupId).executeTakeFirst()) throw new ServiceError(404, '号池分组不存在');
    const saved = await this.#credentials.save({ accountId: row.account_id, workspaceId: row.workspace_id, kind: 'oauth', fileName: `${account.email}-${workspace.external_id}-oauth.json`, content: Buffer.from(JSON.stringify(credential)), poolGroupId: poolGroupId ?? null });
    await this.db.updateTable('codex_oauth_sessions').set({ consumed_at: new Date() }).where('id', '=', row.id).execute();await this.#activity.log({accountId:row.account_id,workspaceId:row.workspace_id,kind:'credential_oauth_created',payload:{credentialId:saved.id,poolGroupId:poolGroupId??null}}); return { id: saved.id };
  }
  ...
  async createPat(accountId: string, workspaceId: string, input: { name?: string; ttl?: number; poolGroupId?: string }) {
    try {
      ...
      const poolGroupId = input.poolGroupId?.trim() || null;
      if (poolGroupId && !await this.db.selectFrom('credential_pool_groups').select('id').where('id', '=', poolGroupId).executeTakeFirst()) throw new ServiceError(404, '号池分组不存在');
      ...
      await this.#activity.log({accountId,workspaceId,kind:'credential_pat_created',payload:{credentialId:row.id,poolGroupId,expiresAt:credential.expired}});
      return { id: row.id };
