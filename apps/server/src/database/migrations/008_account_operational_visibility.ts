  ...
  await sql`
    ...
    with account_personal_plan as (
      select a.id account_id,
        coalesce(latest_subscription.normalized_plan,
          case
            when lower(coalesce(op.account_manager_plan_code, '')) in ('free','go','plus','pro_5x','pro_20x') then lower(op.account_manager_plan_code)
            ...
            when op.account_manager_plan_code is not null then 'unknown' else 'free' end) personal_plan,
      ...
      select a.id account_id,
        coalesce(latest_subscription.normalized_plan,
          case
            ...
            when op.account_manager_plan_code is not null then 'unknown' else 'free' end) personal_plan,
        op.limit_type,op.profile_status
      from accounts a join personal_spaces ps on ps.account_id=a.id
      join account_operational_profiles op on op.account_id=a.id
      left join lateral (select pss.normalized_plan from personal_subscription_snapshots pss where pss.personal_space_id=ps.id order by pss.observed_at desc,pss.created_at desc limit 1) latest_subscription on true
    ), account_workspace_facts as (
      select a.id account_id,bool_or(wm.normalized_role='owner') filter(where wm.id is not null) has_owner,
        count(wm.id)>0 has_membership,
        bool_or(wm.normalized_role='owner' and (w.normalized_plan='business' or latest_subscription.normalized_plan='business' or latest_billing.normalized_workspace_plan='business')) filter(where wm.id is not null) has_two_seat_owner,
        bool_or(wm.normalized_role='owner' and (w.normalized_plan='business_usage_based' or latest_subscription.normalized_plan='business_usage_based')) filter(where wm.id is not null) has_usage_based_owner
      from accounts a left join workspace_memberships wm on wm.account_id=a.id and wm.status='active' and exists(select 1 from workspaces aw where aw.id=wm.workspace_id and aw.status='active')
      left join workspaces w on w.id=wm.workspace_id and w.status='active'
      left join lateral (select wss.normalized_plan from workspace_subscription_snapshots wss where wss.workspace_id=w.id order by wss.observed_at desc,wss.created_at desc limit 1) latest_subscription on true
      left join lateral (select bs.normalized_workspace_plan from billing_snapshots bs where bs.workspace_id=w.id order by bs.observed_at desc,bs.created_at desc limit 1) latest_billing on true
      group by a.id
    )
    select pp.account_id,pp.personal_plan,
      case when pp.personal_plan in ('go','plus','pro_5x','pro_20x') then pp.personal_plan
