...
function seatFieldLabel(value:string){return({contact:'联系方式',remark:'备注',price:'价格',expiresOn:'到期日',expireReminder:'到期提醒',expireRemove:'到期移除',seatType:'席位类型',status:'关系状态'} as Record<string,string>)[value]??value;}
function seatChangeText(payload:Record<string,unknown>,fields:string[]){const before=record(payload.before),after=record(payload.after);return fields.map(field=>['expiresOn','expireReminder','expireRemove','seatType','status'].includes(field)?`${seatFieldLabel(field)} ${auditValue(before?.[field])} → ${auditValue(after?.[field])}`:seatFieldLabel(field)).join('、');}
