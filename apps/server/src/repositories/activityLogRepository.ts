...
function activityText(kind:string,payload:Record<string,unknown>):[string,string?]{
  ...
  return [map[kind]??'业务记录',resources?`范围：${resources}`:email?`邮箱：${email}${error?`；原因：${error}`:''}${billing}${policyDetail}`:previousEmail?`原邮箱：${previousEmail}`:name?`名称：${name}`:remoteUserId?`成员：${remoteUserId}${billing}${policyDetail}`:promoCode?`优惠码：${promoCode}`:status?`状态：${status}`:undefined];
}
