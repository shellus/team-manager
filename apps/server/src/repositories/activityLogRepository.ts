...
function activityText(kind:string,payload:Record<string,unknown>):[string,string?]{
  ...
  const map:Record<string,string>={accounts_bulk_updated:'批量更新账号',personal_space_refresh:'刷新个人空间',personal_settings_changed:'更新个人设置',account_workspace_relationships_refreshed:'同步账号与 Workspace 关系',local_workspace_removed:'彻底删除本地 Workspace',workspace_invitation_created:'发送 Workspace 邀请',workspace_invitation_revoked:'撤销 Workspace 邀请',workspace_member_removed:'移除 Workspace 成员',workspace_member_seat_changed:'调整成员席位',workspace_member_role_changed:'调整成员角色',workspace_renamed:'重命名 Workspace',workspace_settings_changed:'更新 Workspace 设置',workspace_promotion_applied:'应用 Workspace 优惠码',workspace_promotion_unverified:'Workspace 优惠码等待回读确认',credential_created:'创建凭证',credential_pat_created:'创建 PAT 凭证',credential_oauth_created:'创建 OAuth 凭证',credential_replaced:'替换凭证',credential_status_changed:'调整凭证状态',credential_removed:'删除凭证',credential_deployed:'投放凭证',credential_quota_refreshed:'刷新凭证额度',seat_slot_created:'创建客户席位',seat_slot_updated:'更新客户席位',seat_slot_disabled:'停用客户席位',seat_slot_removed:'删除客户席位',seat_slot_released:'释放客户席位',seat_slot_swap_requested:'人工更换席位账号'};
  ...
  const previousEmail=typeof payload.previousEmail==='string'?payload.previousEmail:undefined;const status=typeof payload.status==='string'?payload.status:undefined;const promoCode=typeof payload.promoCode==='string'?payload.promoCode:undefined;
  ...
  return [map[kind]??'业务记录',resources?`范围：${resources}`:email?`邮箱：${email}${billing}${policyDetail}`:previousEmail?`原邮箱：${previousEmail}`:name?`名称：${name}`:remoteUserId?`成员：${remoteUserId}${billing}${policyDetail}`:promoCode?`优惠码：${promoCode}`:status?`状态：${status}`:undefined];
}
