...
function activityText(kind:string,payload:Record<string,unknown>):[string,string?]{
  ...
  const previousEmail=typeof payload.previousEmail==='string'?payload.previousEmail:undefined;const status=typeof payload.status==='string'?payload.status:undefined;
  ...
  return [map[kind]??'业务记录',resources?`范围：${resources}`:email?`邮箱：${email}${billing}${policyDetail}`:previousEmail?`原邮箱：${previousEmail}`:name?`名称：${name}`:remoteUserId?`成员：${remoteUserId}${billing}${policyDetail}`:status?`状态：${status}`:undefined];
}
