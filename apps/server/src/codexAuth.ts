...
export async function exchangeCodexCallback(options: ExchangeCodexCallbackOptions): Promise<CodexCredentialJson> {
  ...
  try {
    ...
    throw new Error(`Codex token exchange 返回不是 JSON: ${(e as Error).message}`);
  }
...
export function codexCredentialFromTokenResponse(
  ...
  const expiresInSeconds = typeof token.expires_in === 'number' && token.expires_in > 0 ? token.expires_in : 0;
...
export function decodeJwtPayload<T>(token: string): T {
  ...
  try {
    ...
    throw new Error(`JWT payload 解析失败: ${(e as Error).message}`);
  }
}
