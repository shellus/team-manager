...
import { configureTraceArtifactSink, configureTransportRuntime, installCatchAllFetchTracing } from './transport.js';
...
async function main() {
  ...
  serve({ fetch: app.fetch, port: config.port }, (info) => {
    ...
    if (config.jwtSecret === 'dev-insecure-secret-change-me') {
      console.warn('[team-manager] 警告: 使用默认 JWT secret，请在 config.yaml 中修改 server.jwtSecret');
    }
  });
