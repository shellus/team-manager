import { Alert, Button, Card, Form, Input, Select, Space, Statistic, Switch, Table, Tag, Typography } from "antd";
...
function ConfigurationForm({ initial, busy, onSave }: { initial: { workspaceId?: string; workspaceName?: string; promoCode?: string; country?: string; currency?: string }; busy: boolean; onSave: (value: Record<string, unknown>) => void }) { return <Form key={JSON.stringify(initial)} layout="vertical" initialValues={{ country: "US", currency: "USD", ...initial }} onFinish={onSave}><div className="team-order-config-grid"><Form.Item name="promoCode" label="优惠码"><Input allowClear /></Form.Item><Form.Item name="country" label="国家"><Select showSearch options={CHECKOUT_COUNTRY_OPTIONS} /></Form.Item><Form.Item name="currency" label="货币"><Select options={CHECKOUT_CURRENCY_OPTIONS} /></Form.Item><Button htmlType="submit" type="primary" loading={busy}>保存配置</Button></div></Form>; }
