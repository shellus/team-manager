import { Alert, Button, Card, Form, Input, InputNumber, Select, Space, Statistic, Switch, Table, Tag, Typography } from "antd";
import { useEffect, useMemo, useState } from "react";
import { Link, useSearchParams } from "react-router-dom";
import type { SeatType, TeamOrderConfigurationView, TeamOrderDashboardView, TeamOrderMaintenanceView, TeamUpgradeOrderView, UnifiedAccountSummaryView, WorkspaceSummaryView } from "@team-manager/shared";
...
type ProductConfirm = ReturnType<typeof useProductModal>["confirm"];
type ProductMessage = ReturnType<typeof useProductMessage>;
  ...
  return <Space direction="vertical" size={16} className="panel-stack">
    ...
    <LoadBoundary loading={loading} error={error} onRetry={load}>
      ...
      <Card title="维护状态"><FilterBar query={query} status={status} onQuery={value => updateParam("query", value)} onStatus={value => updateParam("status", value)} />
        <Table rowKey="id" dataSource={maintenance} pagination={maintenancePagination} scroll={{ x: 1450 }} columns={maintenanceColumns(run, busy, updateParam, data.configured, productModal.confirm)} />
      </Card>
      <Card title="最近订单"><Table rowKey="id" dataSource={orders} pagination={ordersPagination} scroll={{ x: 2050 }} columns={orderColumns(run, busy, updateParam, data.configured, productModal.confirm, productMessage)} /></Card>
    </LoadBoundary>
    <ProductModal title={`编辑维护关系 · ${edit?.workspaceName ?? edit?.workspaceExternalId ?? ""}`} open={Boolean(edit)} onCancel={() => updateParam("editWorkspace")}>
    ...
    <ProductModal title={`Workspace 历史订单（${history.length}）`} open={Boolean(historyWorkspaceId)} width={1000} onCancel={() => updateParam("historyWorkspace")}>
      <Table rowKey="id" dataSource={history} pagination={historyPagination} scroll={{ x: 1800 }} columns={orderColumns(run, busy, updateParam, data.configured, productModal.confirm, productMessage).filter(column => column.title !== "Workspace")} />
    </ProductModal>
  </Space>;
}
...
function ConfigurationForm({ initial, busy, onSave }: { initial: TeamOrderConfigurationView; busy: boolean; onSave: (value: Record<string, unknown>) => void }) {
  const initialValues = { country: "US", currency: "USD", ...initial, ...seatFormValues(initial, 2) };
  return <Form key={JSON.stringify(initial)} layout="vertical" initialValues={initialValues} onFinish={(value) => onSave(withSeatQuantities(value))}>
    <div className="team-order-config-grid"><Form.Item name="promoCode" label="优惠码"><Input allowClear /></Form.Item><Form.Item name="country" label="国家"><Select showSearch options={CHECKOUT_COUNTRY_OPTIONS} /></Form.Item><Form.Item name="currency" label="货币"><Select options={CHECKOUT_CURRENCY_OPTIONS} /></Form.Item><SeatQuantityFields /><Button htmlType="submit" type="primary" loading={busy}>保存配置</Button></div>
  </Form>;
}

function MaintenanceForm({ initial, workspaces, accounts, busy, lockWorkspace, onSave }: { initial?: Partial<TeamOrderMaintenanceView>; workspaces: WorkspaceSummaryView[]; accounts: UnifiedAccountSummaryView[]; busy: boolean; lockWorkspace?: boolean; onSave: (value: any) => void }) {
  const configuration = initial?.configuration ?? {};
  const initialValues = { workspaceId: initial?.workspaceId, executorAccountId: initial?.executorAccountId, enabled: initial?.enabled ?? true, promoCode: configuration.promoCode, country: configuration.country, currency: configuration.currency, ...seatFormValues(configuration) };
  return <Form key={initial?.id ?? "new"} layout="vertical" initialValues={initialValues} onFinish={(value) => onSave(withSeatQuantities(value, true))}><div className="team-order-config-grid"><Form.Item name="workspaceId" label="Workspace" rules={[{ required: true }]}><Select showSearch disabled={lockWorkspace} optionFilterProp="label" options={workspaces.map(row => ({ value: row.id, label: row.name ?? row.externalId }))} /></Form.Item><Form.Item name="executorAccountId" label="执行账号" rules={[{ required: true }]}><Select showSearch optionFilterProp="label" options={accounts.map(row => ({ value: row.id, label: row.email }))} /></Form.Item><Form.Item name="enabled" label="启用" valuePropName="checked"><Switch /></Form.Item></div><ConfigurationFields/><Button htmlType="submit" type="primary" loading={busy}>保存维护关系</Button></Form>;
}
function ConfigurationFields() { return <div className="team-order-config-grid compact"><Form.Item name="promoCode" label="Workspace 优惠码"><Input allowClear /></Form.Item><Form.Item name="country" label="覆盖国家"><Select allowClear showSearch options={CHECKOUT_COUNTRY_OPTIONS} /></Form.Item><Form.Item name="currency" label="覆盖货币"><Select allowClear options={CHECKOUT_CURRENCY_OPTIONS} /></Form.Item><SeatQuantityFields labelPrefix="覆盖" optional /></div>; }
function SeatQuantityFields({ labelPrefix = "", optional = false }: { labelPrefix?: string; optional?: boolean }) { const rules = optional ? [] : [{ required: true }]; return <><Form.Item name="defaultSeats" label={`${labelPrefix}${SEAT_LABEL.default}`} rules={rules}><InputNumber min={0} precision={0} /></Form.Item><Form.Item name="proliteSeats" label={`${labelPrefix}${SEAT_LABEL.prolite}`} rules={rules}><InputNumber min={0} precision={0} /></Form.Item></>; }

function maintenanceColumns(run: (key: string, action: () => Promise<unknown>) => Promise<void>, busy: string, updateParam: (key: string, value?: string) => void, configured: boolean, confirm: ProductConfirm): any[] { return [
...
function orderColumns(run: (key: string, action: () => Promise<unknown>) => Promise<void>, busy: string, updateParam: (key: string, value?: string) => void, configured: boolean, confirm: ProductConfirm, productMessage: ProductMessage): any[] { return [
  ...
  { title: "操作", fixed: "right", render: (_: unknown, row: TeamUpgradeOrderView) => <Space><Button size="small" disabled={!configured} loading={busy === `retry-${row.id}`} onClick={() => run(`retry-${row.id}`, () => unifiedApi.retryTeamOrder(row.id))}>重试</Button><Button size="small" danger onClick={() => confirm({ title: "删除订单记录？", content: "只删除订单记录，不影响 Workspace。", onOk: () => run(`delete-order-${row.id}`, () => unifiedApi.deleteTeamOrder(row.id)) })}>删除</Button></Space> },
]; }
...
function configurationText(value: TeamOrderConfigurationView){const quantities=value.seatQuantities?.filter((item)=>item.quantity>0).map((item)=>`${SEAT_LABEL[item.seatType]} ${item.quantity}`).join(" / ");return [value.country,value.currency,quantities??(value.seatQuantity&&`${value.seatQuantity} 席位`),value.promoCode&&`优惠码 ${value.promoCode}`].filter(Boolean).join(" · ")||"—";}

function seatFormValues(value: Pick<TeamOrderConfigurationView, "seatQuantity" | "seatQuantities"> = {}, fallback?: number) {
  return {
    defaultSeats: seatCount(value, "default", fallback),
    proliteSeats: seatCount(value, "prolite"),
  };
}
function seatCount(value: Pick<TeamOrderConfigurationView, "seatQuantity" | "seatQuantities">, seatType: SeatType, fallback?: number) {
  const row = value.seatQuantities?.find((item) => item.seatType === seatType);
  if (row) return row.quantity;
  return seatType === "default" && value.seatQuantity !== undefined ? value.seatQuantity : fallback;
}
function withSeatQuantities(value: Record<string, unknown>, optional = false) {
  const rawValues = [value.defaultSeats, value.proliteSeats];
  if (optional && rawValues.every((item) => item === undefined || item === null || item === "")) return value;
  const seatQuantities = [
    { seatType: "default" as const, quantity: Number(value.defaultSeats) || 0 },
    { seatType: "prolite" as const, quantity: Number(value.proliteSeats) || 0 },
  ];
  return { ...value, seatQuantity: seatQuantities.reduce((sum, item) => sum + item.quantity, 0), seatQuantities };
}
