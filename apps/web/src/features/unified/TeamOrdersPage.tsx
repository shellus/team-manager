...
function ConfigurationForm({ initial, busy, onSave }: { initial: { workspaceId?: string; workspaceName?: string; promoCode?: string; country?: string; currency?: string; seatQuantity?: number }; busy: boolean; onSave: (value: Record<string, unknown>) => void }) { return <Form key={JSON.stringify(initial)} layout="vertical" initialValues={{ country: "US", currency: "USD", seatQuantity: 2, ...initial }} onFinish={onSave}><div className="team-order-config-grid"><Form.Item name="promoCode" label="优惠码"><Input allowClear /></Form.Item><Form.Item name="country" label="国家"><Select showSearch options={CHECKOUT_COUNTRY_OPTIONS} /></Form.Item><Form.Item name="currency" label="货币"><Select options={CHECKOUT_CURRENCY_OPTIONS} /></Form.Item><Form.Item name="seatQuantity" label="订单席位数" rules={[{required:true}]}><InputNumber min={1} precision={0} /></Form.Item><Button htmlType="submit" type="primary" loading={busy}>保存配置</Button></div></Form>; }

function MaintenanceForm({ initial, workspaces, accounts, busy, lockWorkspace, onSave }: { initial?: Partial<TeamOrderMaintenanceView>; workspaces: WorkspaceSummaryView[]; accounts: UnifiedAccountSummaryView[]; busy: boolean; lockWorkspace?: boolean; onSave: (value: any) => void }) { return <Form key={initial?.id ?? "new"} layout="vertical" initialValues={{ workspaceId: initial?.workspaceId, executorAccountId: initial?.executorAccountId, enabled: initial?.enabled ?? true, promoCode: initial?.configuration?.promoCode, country: initial?.configuration?.country, currency: initial?.configuration?.currency, seatQuantity: initial?.configuration?.seatQuantity }} onFinish={onSave}><div className="team-order-config-grid"><Form.Item name="workspaceId" label="Workspace" rules={[{ required: true }]}><Select showSearch disabled={lockWorkspace} optionFilterProp="label" options={workspaces.map(row => ({ value: row.id, label: row.name ?? row.externalId }))} /></Form.Item><Form.Item name="executorAccountId" label="执行账号" rules={[{ required: true }]}><Select showSearch optionFilterProp="label" options={accounts.map(row => ({ value: row.id, label: row.email }))} /></Form.Item><Form.Item name="enabled" label="启用" valuePropName="checked"><Switch /></Form.Item></div><ConfigurationFields/><Button htmlType="submit" type="primary" loading={busy}>保存维护关系</Button></Form>; }
function ConfigurationFields() { return <div className="team-order-config-grid compact"><Form.Item name="promoCode" label="Workspace 优惠码"><Input allowClear /></Form.Item><Form.Item name="country" label="覆盖国家"><Select allowClear showSearch options={CHECKOUT_COUNTRY_OPTIONS} /></Form.Item><Form.Item name="currency" label="覆盖货币"><Select allowClear options={CHECKOUT_CURRENCY_OPTIONS} /></Form.Item><Form.Item name="seatQuantity" label="覆盖订单席位数"><InputNumber min={1} precision={0} /></Form.Item></div>; }
...
function configurationText(value:{promoCode?:string;country?:string;currency?:string;seatQuantity?:number}){return [value.country,value.currency,value.seatQuantity&&`${value.seatQuantity} 席位`,value.promoCode&&`优惠码 ${value.promoCode}`].filter(Boolean).join(" · ")||"—";}
