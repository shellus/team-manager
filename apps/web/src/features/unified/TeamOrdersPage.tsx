...
const emptyDashboard: TeamOrderDashboardView = { statistics: { maintenanceCount: 0, runningCount: 0, readyCount: 0, attentionCount: 0 }, globalConfiguration: {}, configurations: [], maintenances: [], orders: [] };
...
export function TeamOrdersPage() {
  ...
  const [error, setError] = useState(""); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState("");
  ...
  return <Space direction="vertical" size={16} className="panel-stack">
    ...
    <LoadBoundary loading={loading} error={error} onRetry={load}>
      ...
      <Card title="维护状态" extra={<FilterBar query={query} status={status} onQuery={value => updateParam("query", value)} onStatus={value => updateParam("status", value)} />}>
        <Table rowKey="id" dataSource={maintenance} scroll={{ x: 1200 }} columns={maintenanceColumns(run, busy, updateParam)} />
      </Card>
      <Card title="最近订单"><Table rowKey="id" dataSource={orders} scroll={{ x: 1400 }} columns={orderColumns(run, busy, updateParam)} /></Card>
    </LoadBoundary>
    ...
    <Modal title={`Workspace 历史订单（${history.length}）`} open={Boolean(historyWorkspaceId)} width={1000} footer={null} onCancel={() => updateParam("historyWorkspace")}>
      <Table rowKey="id" dataSource={history} scroll={{ x: 900 }} columns={orderColumns(run, busy, updateParam).filter(column => column.title !== "Workspace")} />
    </Modal>
  </Space>;
}

function FilterBar({ query, status, onQuery, onStatus }: { query: string; status: string; onQuery: (value: string) => void; onStatus: (value: string) => void }) { return <Space wrap><Input.Search allowClear placeholder="Workspace 或执行账号" value={query} onChange={event => onQuery(event.target.value)} /><Select value={status} onChange={onStatus} options={["all", "scheduled", "running", "ready", "attention", "paused", "failed"].map(value => ({ value, label: value === "all" ? "全部状态" : value }))} /></Space>; }
...
function maintenanceColumns(run: (key: string, action: () => Promise<unknown>) => Promise<void>, busy: string, updateParam: (key: string, value?: string) => void): any[] { return [
  ...
  { title: "下次运行", dataIndex: "nextRunAt", render: formatTime }, { title: "上次成功", dataIndex: "lastSuccessAt", render: formatTime },
  { title: "异常", render: (_: unknown, row: TeamOrderMaintenanceView) => row.lastError ?? row.pauseReason ?? "—" },
  { title: "操作", fixed: "right", render: (_: unknown, row: TeamOrderMaintenanceView) => <Space wrap><Button size="small" onClick={() => updateParam("editWorkspace", row.workspaceId)}>编辑</Button><Button size="small" loading={busy === `run-${row.id}`} onClick={() => run(`run-${row.id}`, () => unifiedApi.runTeamOrders({ workspaceId: row.workspaceId, source: "manual_workspace" }))}>运行</Button><Button size="small" onClick={() => run(`toggle-${row.id}`, () => unifiedApi.controlTeamOrder(row.workspaceId, row.enabled ? "pause" : "resume"))}>{row.enabled ? "暂停" : "恢复"}</Button><Button size="small" danger onClick={() => Modal.confirm({ title: "删除维护关系？", onOk: () => run(`delete-${row.id}`, () => unifiedApi.controlTeamOrder(row.workspaceId, "delete")) })}>删除</Button></Space> },
]; }

function orderColumns(run: (key: string, action: () => Promise<unknown>) => Promise<void>, busy: string, updateParam: (key: string, value?: string) => void): any[] { return [
  ...
  { title: "执行账号", dataIndex: "executorEmail" }, { title: "状态", dataIndex: "status", render: (value: string) => <Tag color={statusColors[value]}>{value}</Tag> },
  { title: "Checkout", render: (_: unknown, row: TeamUpgradeOrderView) => row.checkoutUrl ? <Space><Button size="small" type="primary" onClick={() => window.open(row.checkoutUrl, "_blank", "noopener,noreferrer")}>打开</Button><Button size="small" onClick={() => void navigator.clipboard.writeText(row.checkoutUrl!).then(() => message.success("Checkout 链接已复制"))}>复制</Button></Space> : "—" },
  ...
  { title: "操作", fixed: "right", render: (_: unknown, row: TeamUpgradeOrderView) => <Space><Button size="small" loading={busy === `retry-${row.id}`} onClick={() => run(`retry-${row.id}`, () => unifiedApi.retryTeamOrder(row.id))}>重试</Button><Button size="small" danger onClick={() => Modal.confirm({ title: "删除订单记录？", content: "只删除订单记录，不影响 Workspace。", onOk: () => run(`delete-order-${row.id}`, () => unifiedApi.deleteTeamOrder(row.id)) })}>删除</Button></Space> },
]; }
