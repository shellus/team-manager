...
import { Alert, Button, Card, Descriptions, Progress, Tag, Typography } from 'antd';
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (next.accountId) { navigate(`/accounts/${next.accountId}`, { replace: true }); return; }
        if (!['succeeded', 'failed', 'interrupted'].includes(next.operation.status)) setTimeout(poll, 2000);
