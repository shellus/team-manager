...
type ParentBillingRisk = MemberSeatRisk | { kind: 'invite'; email: string; seat: SeatType };
...
export function ParentRoutes({
  ...
  useEffect(() => {
    if (searchState.modal === 'invite-member') {
      inviteForm.setFieldsValue({ seat: selected?.defaultSeat ?? 'usage_based' });
    }
  ...
  const submitInvite = async (values: InviteValues, confirmBillingRisk = false) => {
    ...
    setLocalError('');
    try {
      const updated = await apiClient.invite(selected.id, email, values.seat, confirmBillingRisk);
      onAccountChanged(updated);
      inviteForm.resetFields();
      closeModal();
    } catch (error) {
      if (isBillingRiskError(error)) {
        openBillingRisk({ kind: 'invite', email, seat: values.seat });
      } else {
        reportLocalError(error);
      }
    } finally {
    ...
    try {
      const updated =
        billingRisk.kind === 'invite'
          ? await apiClient.invite(selected.id, billingRisk.email, billingRisk.seat, true)
    ...
    <div className="workbench">
      ...
      >
        <Form<InviteValues>
          form={inviteForm}
          layout="vertical"
          initialValues={{ seat: selected?.defaultSeat ?? 'usage_based' }}
