...
import { unifiedApi, type OperationDetail } from '../unifiedApi.js';
...
export function OperationDrawer({ operation, open, onClose, onChanged }: { operation?: AccountManagerOperationView; open: boolean; onClose: () => void; onChanged?: () => void }) {
  const [detail, setDetail] = useState<OperationDetail>(); const [loading, setLoading] = useState(false); const [error, setError] = useState(''); const [busy, setBusy] = useState(''); const [cardOpen, setCardOpen] = useState(false);
  const load = async () => { if (!operation) return; setLoading(true); setError(''); try { setDetail(await unifiedApi.operation(operation.id)); } catch (e) { setError((e as Error).message); setDetail(operation); } finally { setLoading(false); } };
  ...
  return <><Drawer title="操作详情与恢复" open={open} onClose={onClose} width={720} destroyOnClose extra={<Button onClick={() => void load()}>刷新状态</Button>}>
    <LoadBoundary loading={loading && !value} error={!value ? error : undefined} onRetry={load}>
      ...
      {value && <Space direction="vertical" size={16} className="panel-stack">
        ...
        {detail?.events && <Table rowKey={(row) => String(row.id ?? row.at)} pagination={false} dataSource={detail.events} scroll={{ x: 700 }} columns={[{ title: '时间', dataIndex: 'at', render: formatTime },{ title: '阶段', dataIndex: 'phase' },{ title: '状态', dataIndex: 'status' },{ title: '消息', dataIndex: 'message' }]} />}
        <JsonViewer title="完整请求、响应与结果" value={{ requestSummary: value.requestSummary, result: value.result, control: value.control }} />
      </Space>}
    </LoadBoundary>
