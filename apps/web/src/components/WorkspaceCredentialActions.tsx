...
import { ProductModal, useProductModal } from "./ProductOverlays.js";
...
export function WorkspaceCredentialActions({ credential, run }: {
  ...
  return <>
    <Space wrap>
      ...
      <Button size="small" danger disabled={credential.status==="active"} onClick={()=>productModal.confirm({title:"删除凭证？",content:"只删除已停用的本地凭证记录和制品引用。",onOk:()=>run(`delete-${credential.id}`,()=>unifiedApi.deleteCredential(credential.id))})}>删除</Button>
    </Space>
    {error&&<Alert type="error" showIcon message={error}/>} 
    <ProductModal title="投放凭证到号池" open={deployOpen} onCancel={()=>setDeployOpen(false)}>
