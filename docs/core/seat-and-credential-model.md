...
`expireReminder` 是客户席位是否参与提前到期通知的显式开关，默认开启；迁移前已经存在的席位统一按开启处理。提醒调度只选择同时满足 `expireReminder=true`、存在到期日、未停用且有关联邮箱的席位。关闭提醒不改变到期日，也不改变到期后的停用或自动移除策略。

客户席位到期日使用北京时间自然日语义：到期日当天全天有效，北京时间进入次日 `00:00` 后的第一次到期扫描才执行停用或自动移除。不得使用 UTC 日期边界把处理时间隐式推迟到北京时间早上 `08:00`。
