...
个人订阅接口对当前账号返回 HTTP 404 且正文 `detail` 精确为 `No subscription found for account` 时，表示该账号没有个人订阅记录，刷新流程将其归一化为 `free` 并写入新的订阅快照；其他 404 或非预期错误仍必须失败，不能把通用上游错误静默当作 Free。
...
| `prolite` | Business Premium 固定席位，作为独立计价行 |

席位类型是可缺失的上游事实，不设本地默认值。创建邀请时未显式选择席位，发往上游的请求必须省略席位字段，由上游决定；成员、邀请、Workspace 设置或客户资料的上游响应未返回席位类型时，本地保持未知，界面不显示席位类型。只有明确收到或由管理员明确提交 `default`、`usage_based` 或 `prolite` 时才记录对应类型。

`SeatSlot` 是 Workspace 下已运营、已售出或已明确预留给客户的本地资源。它可以处于待认领状态，此时没有当前邮箱但已经具有稳定公开 `seatKey`；首次认领、后续换号、邀请转成员或席位类型变化不改变客户备注、价格、到期日、公开 `seatKey` 和历史。普通成员移除或邀请撤销只改变远端关系，客户资料继续作为未关联资料显示；只有显式“释放席位”才同时处理远端关系并删除客户资料，空置资料也可由管理员显式删除。

`SeatSlot` 不保存关系 `status` 或远端用户 ID。待认领、成员、邀请中和未关联状态必须由当前邮箱与活动 Membership、待接受 Invitation 实时派生；到期状态必须由到期日和北京时间业务日期实时派生。客户资料存在、远端资格存在和是否已到期是三个独立事实，不能互相覆盖或删除。

`expireReminder` 是客户席位是否参与提前到期通知的显式开关，默认开启；迁移前已经存在的席位统一按开启处理。提醒调度只选择同时满足 `expireReminder=true` 和存在到期日的席位，待认领席位也可提醒。关闭提醒不改变到期日、到期状态或到期自动移除策略。提醒正文逐项说明到期后是否自动移除，不能只发送数量。

客户席位到期日使用北京时间自然日语义：到期日当天全天有效，北京时间进入次日 `00:00` 后派生为已到期。不得使用 UTC 日期边界把处理时间隐式推迟到北京时间早上 `08:00`。

`expireRemove=false` 表示到期只显示为已到期，不修改远端关系，也不修改或隐藏本地客户资料。`expireRemove=true` 表示到期后尝试移除对应远端成员或邀请：首次立即执行，失败后分别等待 1 分钟和 5 分钟，最多尝试 3 次。成功后保存成功终态并保留客户资料；最终失败时保存失败终态、停止自动重试、保留原关系和客户资料，并通过 `seat_expiration` 通知策略告警。普通到期扫描不得重新调度成功或最终失败的移除；管理员修改到期日或到期移除策略后才清除该终态。

到期通知必须是可执行的运营摘要，不得只发送数量：正文包含统计时间、提醒窗口、按到期日排序的 Workspace 与客户邮箱、剩余天数、相对上一次成功通知的新增/移出提醒范围数量，以及可选管理入口。正文最多展开 10 项并按跨渠道 1800 UTF-8 字节上限截断；完整结构化 `items` 仍保存在通用 Webhook 与投递记录中。自动移除最终失败使用独立告警正文，明确有限重试已经耗尽、原关系仍保留并要求人工处理。

通知投递在入队时冻结渠道配置，并逐渠道保存成功状态。重试仍限制为 3 次，且只重试尚未成功的渠道，不能向已经成功的渠道重复发送。通用 Webhook 以 HTTP 成功为准；飞书、企业微信和 Telegram 还必须校验响应体业务成功码。策略中的管理入口必须是显式 HTTP(S) URL。发送测试使用与正式通知相同的正文模板和渠道协议，但必须标识为测试并只包含示例数据。

客户席位编辑活动必须记录 `expiresOn`、`expireReminder`、`expireRemove` 等可变运营字段的前后值和变更字段列表，保证通知名单变化可以回溯，不能只记录“席位已更新”。

Workspace 固定席位空位始终由容量与远端关系实时派生，不能批量物化成无主 `SeatSlot`。管理员只能从已知固定 ChatGPT 或 Premium 空位中显式创建待认领席位；待认领席位减少本地可分配固定席位余量，但在创建上游邀请前不计入 `fixedSeatOccupied`。容量未知或没有可分配余量时不得创建待认领席位。Codex/`usage_based` 不使用待认领席位或固定席位空位模型。

公开 URL 是待认领席位的能力凭据，其存在即允许首次认领，不另设首次认领开关。首次认领提交首个邮箱并创建 `default` 固定 ChatGPT 邀请；成功后立即开始换号冷却期。未绑定邮箱时不能导出数据。

后续公开换号默认关闭，只有管理员为对应 `SeatSlot` 显式允许后才能执行。换号冷却默认 7 天，按最近一次成功的公开认领或公开换号完成时间计算；失败、相同邮箱幂等请求和管理员操作不重置冷却。已到期、缺少可管理账号或已有活动自助操作的席位拒绝认领、换号和导出。
...
| `fixedSeatOccupied` | 当前固定席位关系占用 | 活动 `default`/`prolite` Membership + 待接受 `default`/`prolite` Invitation | 无匹配关系时为 0 |
