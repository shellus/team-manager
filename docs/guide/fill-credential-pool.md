...
1. 在账号列表筛选目标账号，再在账号详情的 Workspace 标签选择本轮空间；根据外部运营记录排除已出租客户席位和仍需保留额度的成员。
2. 刷新成员、邀请和设置。
...
在账号页启动 GAM 注册，创建时选择目标账号分组。注册成功后导入 GAM 交付的 Web Session，再由 Team Manager 直连刷新个人空间和账号与 Workspace 关系。注册任务异常在账号操作记录和 GAM 中处理。
