...
已出租客户席位以 `seatSlots` 和成员本地资料为准；客户资料可关联 `default` 或 `usage_based`。`usage_based` 成员不占固定 ChatGPT 席位，但如果该账号仍需要保留 Team membership、客户资料或凭证，不应移除。
...
3. 对已经确定不再复用该 Team 凭证的旧成员，优先评估能否切到 `usage_based`；只有明确需要解除 membership 时才执行“移出成员”。
4. 如果旧成员只是暂时不用额度、后续还要复用同 Team 凭证，应优先切到 `usage_based`，不要移除。
5. 每次移出或改席位后，重新刷新成员列表，并检查 Workspace Billing、最近一次成员移除策略结果和 `default` 数量。

移除成员会破坏该账号在该 Team 下的 membership，可能使原 Team 凭证不可用。标准 ChatGPT 成员被移除后，对应席位仍可能临时计费；新成员可能形成独立付费席位，高频席位分配滥用还可能导致 Workspace 停用或账号暂停。只有在明确跨 Team 搬迁或清理无用成员时才使用移除，不把“移除后重新邀请”作为重置使用限制的自动轮转手段。
...
3. 邀请或切换到 `default` 前，人工确认当前 `default` 成员、临时计费席位和 Billing；系统不会为普通席位修改增加二次确认。
