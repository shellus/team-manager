...
成员和待接受邀请在同一个列表中显示，并按邮箱关联客户联系方式、备注、价格和到期日；失去远端关系但仍保留的客户资料也继续显示。设置到期日即自动参与提醒，不另设提醒开关。当前账号是 owner/admin 时可以刷新关系、邀请、撤销、移除、改角色/席位、编辑或释放客户资料并修改设置；普通成员只能查看缓存资料并管理自己的凭证。订阅、续费、金额、计费席位、支付方式和发票统一在账单子标签中显示。Workspace 活动日志不在该界面展示。
