# 业务流程总览

team-manager 的业务流程围绕四个对象展开：母号、Team workspace、子号和 Codex 凭证。母号负责管理 Team；子号负责加入 Team 并使用该 Team 下的 Codex 凭证；席位类型决定账单风险和额度来源。

## 核心流程

1. 录入母号 session。
2. 刷新母号状态，确认 Team 名称、成员、待处理邀请和默认席位。
3. 将新成员默认席位设置为 Codex 席位，并按需调整“允许成员发送 Codex 邀请”等 Team 权限开关，降低普通成员误邀造成 ChatGPT 固定席位超额的风险。
4. 邀请成员或子号邮箱进入目标 Team，按业务需要选择 ChatGPT 席位或 Codex 席位。
5. 在子号页自动注册账号，或录入子号 session。
6. 同步子号 Team 关联，确认该子号在目标 Team 中是 `member` 或 `invited`。
7. 对每个目标 Team 创建对应 PAT。
8. 刷新额度，查看该凭证在对应 Team workspace 下的额度窗口。

需要用新号批量填充 CPA/Codex 号池时，按[新号填充凭证号池 SOP](./fill-credential-pool)执行。该流程要求先确认可用母号和席位边界，再让新子号加入目标 Team，并按“子号 × Team workspace”生成 PAT 凭证。

## 页面入口

| 页面 | 主要用途 | 常见动作 |
|---|---|---|
| 母号 | Team workspace 管理 | 录入母号、刷新、邀请成员、在统一成员列表查看成员/邀请/客户席位、修改默认席位、设置 Codex 邀请权限、设置个人访问令牌权限、改成员席位、移出成员、释放失联客户席位 |
| 子号 | 子号池和 Codex 凭证管理 | 自动注册、录入子号、编辑本地资料、同步 Team 关联、OAuth 授权、创建 PAT、刷新额度、下载凭证 |
| 订单维护 | Codex Workspace 的 Team 升级订单池 | 保存全局配置、查看有效链接、立即生成单个或全部订单、暂停维护 |

母号列表的关键词搜索框下方提供快捷标签筛选，可按 GAM、0.52、双席位、双席位限额类型、封号和订单维护状态组合过滤。GAM、0.52、双席位、封号和订单维护都是独立二态维度，统一使用“是 / 否”且每个维度只能选择一个状态；不同维度按同时满足，例如可以组合出“0.52 是、双席位 否”。周限 / 月限 / 未知可多选并按任一匹配。筛选值会写入当前 URL 并保存到浏览器本地，刷新或切换页面后返回母号页仍会恢复。

列表只展示已经开通的 0.52 和双席位标签，不展示“未开”标签。周限、月限和未知限额标签只属于双席位 Team；未开双席位的母号不显示任何限额标签，也不会命中限额快捷筛选。

## 业务边界

- 管理动作必须通过页面、API 或 service/store 方法完成，不通过编辑运行时 JSON 文件生效。
- 成员数、ChatGPT 席位数、pending invite 数和列表标签从当前缓存派生，不作为独立数据维护。
- Codex 凭证按“子号 × Team workspace”保存。同一子号加入多个 Team 时，需要保留多份凭证。
- GongXi-Mail、CloakBrowser、Mihomo 和家宽代理属于独立注册服务。Team Manager 只保存注册服务地址与调用 Token。

## 席位类型

| 席位 | 原始值 | 业务含义 |
|---|---|---|
| ChatGPT 席位 | `default` | 占用 Team 固定席位，有 Team 额度窗口，可能产生额外账单 |
| Codex 席位 | `usage_based` | 不占用固定 ChatGPT 席位，不提供 ChatGPT 席位额度 |

邀请成员或把成员切换到 ChatGPT 席位时，系统不会增加二次确认。操作员必须结合 Workspace Billing 判断；标准成员移除后可能继续临时计费，当前成员和邀请列表不是计费席位的完整事实源。

## 推荐日常顺序

1. 先在母号页刷新目标 Team。
2. 确认默认席位是 Codex 席位，并按需关闭成员发送 Codex 邀请。
3. 邀请或调整成员前，先查看 ChatGPT 席位已用数量。
4. 子号加入 Team 后，在子号页同步 Team 关联。
5. 按目标 Team 创建 PAT。
6. 刷新额度并根据额度窗口调整席位类型，不使用高频“移除后重新邀请”来重置限制。
