...
自动付款不能确认成功或付款页面需要人工检查时，对应的 0.52、双席位或 Pro 5x 操作进入“等待人工处理”。Account Manager 保留原 Checkout 页面并继续监听；Checkout 页面首次进入或刷新后会自动补填一次，操作员可在提交前修改。Pro 5x 浏览器自动化在进入站内 Checkout 前中断时也保留当前 GAM Profile 页面现场。等待人工的 Pro 5x 任务可选择“重试当前步骤”保留当前 SID/IP 重新接管，或选择“更换 IP 并重试”轮换住宅出口。服务热重载后恢复加密付款请求和任务监听，并按当前页面状态继续自动化。
...
“录入母号”仍使用当前选中的母号分组；当前选中“所有”时归入 `默认分组`。“自动注册”在提交前单独选择注册国家和所属分组，允许输入新分组。服务端保存最后一次提交值，下次打开表单自动回填；目标国家和分组跟随持久化任务，即使页面或服务重启，交付后也不会丢失归属。
...
优惠码开关和值由 Team Manager 服务端保存，默认勾选并填入 `stb`。每次新建 Pro 5x 任务后，下次打开表单回填最近一次提交值；关闭开关时仍保留优惠码输入值，但 GAM 创建 ChatGPT Checkout 的请求不携带 `promo_code`。
...
母号详情把成员、待处理邀请和本地客户席位按规范化邮箱合并到同一个“成员”列表，同一邮箱只显示一行。页面先显示已有缓存；“刷新成员与邀请”会依次刷新两份远端关系快照，默认席位继续在设置页独立刷新。
...
“成员”列表统一展示正式成员、待处理邀请和本地客户席位。同一邮箱的远端成员或邀请关系与 `seatSlots` 客户资料合并为一行，状态分别显示“成员”“邀请中”“关系失联”或“空位”。邀请接受后，同一行从“邀请中”迁移为“成员”，不会新建第二条客户资料。

待处理邀请仍与正式 member 保持不同业务状态。子号处于 pending invite 时，仍可能为目标 Team 创建 PAT；本地 Team 关联状态保持为 `invited`，直到同步到正式成员关系。邀请行可撤销邀请，也可编辑客户席位资料；编辑只更新 Team Manager 本地数据，不调用 ChatGPT。

成员和邀请的两份成功快照都找不到客户席位当前邮箱时，该行显示“关系失联”。系统继续保留备注、到期时间、价格、管理地址和换号历史，不自动删除。操作员确认客户关系已经结束后，可在该行手动释放本地客户席位；固定 ChatGPT 席位释放后才在概览中成为真正空位。

## 成员席位和移出成员

统一成员列表支持修改正式成员的席位和角色。系统不检查切换后的固定席位数量，由操作员自行判断。
