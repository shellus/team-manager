...
推荐使用“自动注册”。提交前选择注册国家和所属分组，所属分组既可选择已有值，也可直接输入新值。服务端保存最后一次提交值，下次打开表单自动回填；任务创建时即把国家应用到 GAM 住宅代理，并把目标分组写入持久化任务。按钮发起任务后立即释放，子号列表出现独立任务项并展示进度，刷新页面不会丢失。
...
优惠码开关和值与母号共用同一份服务端偏好，默认勾选并填入 `stb`。新建任务后保存本次提交值；关闭开关时实际 ChatGPT Checkout 不携带 `promo_code`。
