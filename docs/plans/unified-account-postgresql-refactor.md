...
状态：领域模型、数据库和一次性迁移已完成；产品功能补全尚未完成。后续实施与验收以 [`unified-account-product-completion.md`](./unified-account-product-completion.md) 为准。
...
| 阶段 | 状态 | 完成提交 | 验证记录 |
|---|---|---|---|
| 阶段 0：计划确认与上游协议观测 | 已完成 | `9156c23` | 分组冲突采用历史管理账号侧分组；付费套餐互切未取得证据，产品门禁为禁用并返回 409，不依赖猜测 |
...
- 完整上游 HTTP trace 文件：迁移历史段可保持 JSONL，新运行记录按请求生成不可变 JSON 文件；
- rrweb `json.gz` 压缩录制；
...
迁移进来的 HTTP trace 段保持原 JSONL 文件；新版运行记录按请求生成不可变 JSON 文件。数据库只保存按时间、请求来源或操作 ID 检索所需的文件索引，不复制正文。rrweb 延续既有保留周期，凭证文件不因业务记录停用而立即物理删除。
...
结果用于 Workspace 写操作的执行账号选择、关系详情和授权校验。不得写回账号表，也不得据此改变账号分组。账号列表以“主套餐”承担运营称呼，不再把可管理空间或普通成员作为列表字段和筛选。

账号列表支持以下 URL 可恢复筛选：

- 账号分组；
- GAM 已关联：“是/否”两枚互斥 Checkbox，均不选表示所有；
- Profile 运行中：“是/否”两枚互斥 Checkbox，均不选表示所有；
- 主套餐；
- 显示人工封号：默认不勾选并隐藏，勾选后同时显示并使用错误状态徽标；
- 关键词。

账号列表字段为账号、分组、主套餐、能力和操作。“能力”只在账号已绑定 GAM 时显示 `GAM` 标签；Workspace 数量、凭证数量、可管理空间、普通成员、凭证和 Session 不在列表重复展示。
...
- 旧 HTTP trace 保持原 JSONL 文件并导入文件段索引；新版 HTTP trace 按请求写入不可变 JSON 文件，数据库只建立文件索引。
...
- 账号列表主套餐、GAM/Profile 勾选筛选和封号错误状态徽标；
...
- 付费个人套餐相互切换仍未取得上游只读证据，UI 禁用且 API 返回 409；
- 产生扣款、真实成员移除或真实套餐变化的动作未用于自动生产冒烟；
- HTTP trace、rrweb 和凭证正文全部保持文件制品，结构化业务状态只在 PostgreSQL。
...
本文已获得正式实施确认并按台账实施。后续新增范围或领域模型变化仍须先回写稳定文档和 ADR。
