...
- [x] B1. 账号列表实现展开的分组快捷筛选；每个分组数量基于当前关键词、主套餐、GAM、Profile 运行与封号等其他筛选条件实时计算，切换分组不会改变计数基准。主套餐保持 URL 可恢复选择；GAM、Profile 运行与封号各使用“是/否”两枚互斥 Checkbox，均不选表示所有，URL 保存最终三态。可管理空间、普通成员、凭证、Session 及 Workspace/凭证数量不再出现在列表筛选或字段中；封号账号在账号字段使用错误状态徽标标识。
