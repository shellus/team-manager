...
- Workspace 订阅、结构化账单/发票、全部已支持设置的单项更新，以及成员/邀请变更后的凭证资格重算；
- SeatSlot CRUD、释放、人工/公开换号历史、到期提醒去重、派生到期状态和可选移除；
...
- [x] B1. 账号列表实现展开的分组快捷筛选；每个分组数量基于当前关键词、主套餐、GAM、Profile 运行与封号等其他筛选条件实时计算，切换分组不会改变计数基准。主套餐保持 URL 可恢复选择，双席位使用单个 `双席位 n/2` Tag 显示代表 Workspace 的 ChatGPT 席位占用。关键词、分组、主套餐和三态条件同时保存为 localStorage 筛选记忆，显式 URL 优先，无筛选参数进入列表时恢复记忆，重置时清除。GAM、Profile 运行与封号各使用“是/否”两枚互斥 Checkbox，均不选表示所有，URL 保存最终三态。可管理空间、普通成员、凭证、Session 及 Workspace/凭证数量不再出现在列表筛选或字段中；封号账号在账号字段使用错误状态徽标标识。多选模式支持跨分页全选当前筛选结果，并通过单一事务批量更换分组、标记封号或取消封号；筛选变化、分组变化或退出多选时清空选择，注册临时行不可选择。
...
- [x] B8. AccountGroup 支持通过拖动手柄稳定排序，保存失败时恢复原顺序。

### 阶段 C：Workspace

- [x] C1. Workspace 订阅实时刷新并保存快照。
- [x] C2. Workspace 账单结构化解析席位、金额、周期、续费、支付方式、Upcoming invoice 和发票；原始证据留在后端快照，不进入 Web UI。
- [x] C3. Workspace 设置提供全部已支持字段的结构化控件；名称独立保存，其余设置单项即时提交。每次请求只写一个上游设置，成功后把提交值合并进本地快照，不自动回读上游。
- [x] C4. 成员角色和席位使用表格内独立下拉框修改，每次请求只写一个上游字段，成功后直接更新本地关系；邀请默认使用 ChatGPT 席位和分析者角色，移除和撤销失败可恢复。
- [x] C5. Account 内 Workspace 上下文路由可直接访问，Tab、执行账号和弹窗状态可刷新恢复。
- [x] C6. `/parent-overview` 只展示管理账号未封号的双席位 Business Workspace；卡片标题单行显示直接状态、账号备注和管理账号，不显示 Workspace 名称。`/seat-overview` 只展示固定 ChatGPT 席位 Workspace 的固定成员、固定邀请、容量空位及其可选客户资料，不展示 Codex 空间。两者是顶部导航中的独立一级页面，使用可筛选、可分页的响应式 Grid；数据 API 分别为 `/api/overview/renewals` 和 `/api/overview/seats`。

### 阶段 D：客户席位

- [x] D1. SeatSlot 可创建、编辑和删除，字段包含邮箱、联系方式、备注、价格、到期日、席位类型和稳定访问键；不存在本地停用状态。
- [x] D2. SeatSlot 支持释放失联占用、复制公开链接和管理员人工换号。
- [x] D3. 到期提醒、派生到期状态和可选远端移除策略由持久后台任务执行；成功或最终失败后不再调度。
