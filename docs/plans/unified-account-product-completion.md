...
- [x] B1. 账号列表实现展开的分组快捷筛选；每个分组数量基于当前关键词、套餐、能力与封号可见性等其他筛选条件实时计算，切换分组不会改变计数基准。可管理空间、普通成员、凭证、GAM、Profile、Session 和套餐保持 URL 可恢复三态筛选；“显示封号”使用 URL 可恢复勾选项，默认不勾选并隐藏人工封号账号，勾选后同时显示正常与封号账号。
