...
- [x] B1. 账号列表实现展开的分组快捷筛选；每个分组数量基于当前关键词、主套餐、GAM、Profile 运行与封号可见性等其他筛选条件实时计算，切换分组不会改变计数基准。主套餐保持 URL 可恢复选择；GAM 与 Profile 运行使用 URL 可恢复勾选筛选，不勾选表示不限。可管理空间、普通成员、凭证、Session 及 Workspace/凭证数量不再出现在列表筛选或字段中。“显示封号”默认不勾选并隐藏人工封号账号，勾选后同时显示正常与封号账号，并在账号字段使用错误状态徽标标识。
