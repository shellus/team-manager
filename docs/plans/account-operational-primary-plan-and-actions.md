...
- 既有分组快捷筛选、动态数量、能力筛选和默认隐藏人工封号行为保持不变；分组数量继续结合主套餐等其他筛选条件计算。
...
- [ ] `accounts` 表没有主套餐缓存列、触发器或双写服务。
- [ ] `account_operational_summaries` 能在 PostgreSQL 中返回并筛选全部主套餐值。
- [ ] 个人付费套餐总是覆盖 Workspace 称呼。
- [ ] 双席位覆盖 0.52；仅 owner 关系参与这两项判定。
- [ ] 只有非 owner 活动关系时返回 Team 子号，admin 包含在内；owner 与非 owner 混合时不返回 Team 子号。
- [ ] 已移除关系、Invitation 和非活动 Workspace 不参与判定。
- [ ] 明确 Free 与证据不足的 `unknown` 可区分。
- [ ] 列表查询不接受旧 `personalPlan` 参数，也不在应用内存中过滤主套餐。

### 列表与详情

- [ ] 账号邮箱是唯一详情链接；点击整行、备注、套餐和操作不会跳转。
- [ ] 邮箱下只显示账号备注，空备注显示 `—`。
- [ ] 主套餐列和筛选仅显示最终称呼，不展示来源或 Workspace 明细。
- [ ] 分组数量结合主套餐和其他筛选实时计算，筛选与弹窗状态刷新不丢失。
- [ ] 操作列平铺“启动/停止、换 IP、开通、编辑”，没有下拉菜单。
- [ ] 列表与详情使用相同组件和 API，不保留第二套代理、开通或 Session 表单。
- [ ] 换 IP 弹窗包含 SID、国家、ASN、州/省和城市全部字段。
- [ ] 开通弹窗涵盖个人四档套餐及 Business 两种模式。
- [ ] 编辑弹窗打开即加载完整 Session，可保存且不脱敏；没有单独查看或下载入口。
- [ ] 未绑定 GAM、动作执行中、上游失败和成功刷新均有一致反馈。
