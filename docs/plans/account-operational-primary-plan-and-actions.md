...
- 分组列位于账号列之前，两列均固定在表格左侧。
- 分组管理使用独立拖动手柄排序，支持鼠标、触摸和键盘操作；不保留上下移动按钮，保存失败时恢复原顺序。
- 账号列主标题是邮箱，且只有邮箱 `<a>` 链接进入 `/accounts/:accountId`；整行和其他字段不触发详情跳转。
- 账号列二级标题只显示账号备注，空值留空；不使用 `—`、Workspace 名称、显示名或其他字段回退。
- 原“套餐”列改为“主套餐”，原 `personalPlan` 列表筛选改为 `primaryPlan`。
- 主套餐为“双席位”时，使用单个不换行的 `双席位 n/2` Tag；`n` 统计活动与邀请中的 `default` ChatGPT 席位，未满显示黄色、满席显示绿色、超出显示红色。
- 分组快捷筛选与动态数量保持不变；分组数量继续结合主套餐、GAM、Profile 运行和封号等其他筛选条件计算。
- 关键词输入过程不触发查询，仅在回车或输入框失焦时提交；筛选栏提供“刷新”和“重置”，重置同时清空未提交关键词与全部 URL 筛选。
- 关键词、分组、主套餐、GAM、Profile 运行和封号筛选继续写入 URL，同时保存到 localStorage。显式 URL 筛选优先；从顶部导航进入无筛选参数的 `/accounts` 时恢复上次筛选。“重置”同时清除 URL 和筛选记忆；页码、每页数量、弹窗和操作状态不进入筛选记忆。
- 正式账号在没有显式排序时按创建时间正序排列，最早创建的账号在前；创建时间不作为列表列展示。分组、账号、主套餐、续费/到期和能力表头支持“正序、倒序、取消”三态排序，取消后回到默认创建时间正序。编辑账号、换分组、封号、Profile 状态变化或批量操作不会因更新时间变化而改变顺序。
- 列表“能力”只显示已绑定的 GAM 标签；不再重复展示可管理空间、普通成员、凭证和 Session，也不显示 Workspace 与凭证数量。
- GAM、Profile 运行与封号各使用“是/否”两枚互斥 Checkbox 表达三态：均不选表示所有，选中的再次点击恢复所有。可管理空间、普通成员、凭证和 Session 不再提供列表筛选。
- 封号账号在账号字段中用错误状态徽标标识，不占用“能力”字段。
...
| 编辑 | 打开单页账号编辑弹窗，铺开分组、账号备注、限额类型、封号标记和完整 Session；Session 可编辑并保存，不脱敏，不再设置查看或下载入口；弹窗提供保存和取消操作 |

列表与账号详情必须复用同一组操作按钮、弹窗、忙碌状态、错误处理和成功后刷新逻辑。依赖 GAM 的动作在未绑定时禁用并说明原因。弹窗由 URL 参数表示，刷新后恢复；操作按钮必须阻止账号链接或其他导航事件。

## 批量选择与账号级操作

账号列表默认保持现有单账号操作密度。点击“多选”后才显示行 Checkbox、全选筛选结果、已选数量和批量按钮；注册中的临时账号不是正式 `Account`，始终不可选择。分页切换保留选择，关键词、主套餐、三态条件或分组变化会清空选择，退出多选也会清空选择。

“全选筛选结果”覆盖当前全部筛选结果中的正式账号，不局限于当前分页。首批批量动作平铺显示“更换分组、标记封号、取消封号”，不再套下拉菜单。更换分组使用弹窗选择目标结构化 `AccountGroup`；封号状态操作是可逆账号字段更新。

前端只提交一份账号 ID 集合和目标补丁，后端通过 `PATCH /api/accounts/bulk` 在单一 PostgreSQL 事务中校验全部账号、目标分组并执行批量更新。任一账号或分组无效时整体失败，不允许前端循环调用单账号接口造成部分成功。事务同时写入一条批量活动日志，但不复制主套餐、Workspace 能力或其他派生事实。
...
- [x] 分组列位于账号列之前，两列横向滚动时保持固定。
- [x] 分组管理通过拖动手柄即时排序并保存，失败时回滚，不显示上下移动按钮。
- [x] 账号邮箱是唯一详情链接；点击整行、备注、套餐和操作不会跳转。
- [x] 邮箱下只显示账号备注，空备注留空且不显示占位符。
- [x] 主套餐列和筛选仅显示最终称呼，不展示来源或 Workspace 明细。
- [x] 双席位主套餐使用单个不换行的 `双席位 n/2` Tag，并按未满、满席、超出使用黄、绿、红状态色。
- [x] 分组数量结合主套餐和其他筛选实时计算，筛选与弹窗状态刷新不丢失。
- [x] 关键词仅在回车或失焦时查询；刷新重新加载当前条件，重置清空关键词、筛选和对应 URL 参数。
- [x] 显式 URL 筛选优先；无筛选参数进入账号列表时从 localStorage 恢复上次筛选，重置同时清除记忆，且不保存分页、弹窗或操作参数。
- [x] 正式账号默认按创建时间正序稳定排列且不显示创建时间列；可见业务字段支持表头正序、倒序和取消排序，取消后恢复默认顺序；更新时间和 Profile 运行状态不参与默认排序。
- [x] 能力列只在已绑定时显示 GAM；账号列用错误状态徽标标识人工封号。
- [x] 列表不显示 Workspace/凭证数量，也不提供可管理空间、普通成员、凭证和 Session 筛选。
- [x] GAM、Profile 运行与封号使用“是/否”两枚互斥 Checkbox 表达所有/是/否三态。
...
- [x] 编辑弹窗单页铺开分组、账号备注、限额类型、封号标记和完整 Session；Session 编辑器固定 15 行、约 260px 高，自动换行并在内容超出时内部纵向滚动，可保存且不脱敏，没有单独查看或下载入口；弹窗提供保存和取消按钮。
- [x] 未绑定 GAM、动作执行中、上游失败和成功刷新均有一致反馈。
- [x] 点击“多选”后才显示账号 Checkbox、全选筛选结果、已选数量和批量按钮；退出后清空选择。
- [x] 全选覆盖全部筛选结果并跨分页保持选择，筛选或分组变化清空选择；注册临时行不可选择。
- [x] 批量更换分组、标记封号和取消封号使用一个事务接口，任一目标无效时整体回滚。
