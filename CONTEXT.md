...
**客户席位到期日**:
客户席位在北京时间中保持有效的最后一个自然日。到期日结束、北京时间进入次日零点后，席位才进入到期停用或自动移除流程。
_Avoid_: UTC 到期日、精确到秒的到期时间、北京时间早上八点到期

**到期自动移除**:
客户席位到期后必须完成对应远端成员或邀请移除的处理策略。达到有限尝试次数仍未移除时进入显式失败状态并保留原关系和客户资料，不能等同于到期停用。
_Avoid_: 到期停用、静默降级、无限自动重试

**到期提醒**:
客户席位是否参与提前到期通知的显式选择。只有提醒开启且存在到期日时，席位才进入提醒调度；该选择不改变到期后的停用或自动移除策略。
